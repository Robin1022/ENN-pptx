...
from helpers import safe_extract, write_minidom_xml
...
class BaseSchemaValidator:
    ...
    def repair_whitespace_preservation(self) -> int:
        ...
        for xml_file in self.xml_files:
            try:
                ...
                if pending:
                    write_minidom_xml(xml_file, dom)
    ...
    def validate_standalone_declaration(self):
        errors = []
        ...
        for xml_file in self.xml_files:
            ...
            if decl_end == -1:
                continue
            if b"standalone" not in head[:decl_end]:
                errors.append(f"  {xml_file.relative_to(self.unpacked_dir)}")

        if errors:
            print(
                f'FAILED - {len(errors)} part(s) missing standalone="yes" in the XML '
                f"declaration (PowerPoint's strict parser treats these as damaged and "
                f"silently drops their content on open — passes XSD/LibreOffice/python-pptx "
                f"fine, so this is easy to miss without this check):"
            )
            for error in errors:
                print(error)
            print(
                "  Fix: a minidom edit wrote these without going through write_minidom_xml() "
                "-- don't call Document.toxml(encoding=...) directly (it silently drops "
                'standalone); use dom.documentElement.toxml() and prepend '
                'b\'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\\r\\n\' yourself, '
                "or reuse the write_minidom_xml() helper in this module."
            )
            return False
        if self.verbose:
            print('PASSED - All XML parts declare standalone="yes"')
        return True
