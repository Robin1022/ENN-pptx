...
def write_minidom_xml(path, dom) -> None:
    """Serialize a minidom Document back to disk without losing standalone="yes".

    Document.toxml(encoding=...) silently drops standalone="yes" from the XML
    declaration. Every OOXML part written by PowerPoint/WPS has it; a part missing
    it still passes XSD validation, python-pptx, and LibreOffice, but real
    PowerPoint's strict parser treats the part as damaged and silently drops its
    content on open. Use this (or plain read_bytes()/write_bytes(), never
    read_text()/write_text()) for any part you edit — Python's text-mode I/O also
    silently downgrades the \\r\\n Office/WPS use after the XML declaration to \\n,
    which trips the same failure mode.
    """
